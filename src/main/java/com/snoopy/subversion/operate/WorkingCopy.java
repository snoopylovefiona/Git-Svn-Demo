/*
 * ==================================================================== Copyright (c) 2004-2008 TMate Software Ltd. All
 * rights reserved. This software is licensed as described in the file COPYING, which you should have received as part
 * of this distribution. The terms are also available at http://svnkit.com/license.html If newer versions of this
 * license are posted there, you may use a newer version instead, at your option.
 * ====================================================================
 */
package com.snoopy.subversion.operate;
...
import com.snoopy.subversion.handler.CommitEventHandler;
import com.snoopy.subversion.handler.InfoHandler;
import com.snoopy.subversion.handler.StatusHandler;
import com.snoopy.subversion.handler.UpdateEventHandler;
import com.snoopy.subversion.handler.WCEventHandler;
...
public class WorkingCopy {
    ...
    public static void main(String[] args) throws SVNException {
        ...
        /**
         * Assuming that 'svn://localhost/testRep' is an existing repository path. SVNURL is a wrapper for URL strings
         * that refer to repository locations.
         */
        ...
        /**
         * That's where a local directory will be imported into. Note that it's not necessary that the '/importDir'
         * directory must already exist - the SVN repository server will take care of creating it.
         */
        ...
        /**
         * Creates a default run-time configuration options driver. Default options created in this way use the
         * Subversion run-time configuration area (for instance, on a Windows platform it can be found in the
         * '%APPDATA%\Subversion' directory). readonly = true - not to save any configuration changes that can be done
         * during the program run to a config file (config settings will only be read to initialize; to enable changes
         * the readonly flag should be set to false). SVNWCUtil is a utility class that creates a default options
         * driver.
         */
        ...
        try {
            /**
             * creates a new version comtrolled directory in a repository and displays what revision the repository was
             * committed to
             */
        ...
        /**
         * creates a new local directory - './importDir' and a new file - './importDir/importFile.txt' that will be
         * imported into the repository into the '/MyRepos/importDir' directory
         */
        ...
        try {
            /**
             * recursively imports an unversioned directory into a repository and displays what revision the repository
             * was committed to
             */
        ...
        try {
            /**
             * recursively checks out a working copy from url into wcDir. SVNRevision.HEAD means the latest revision to
             * be checked out.
             */
        ...
        try {
            ...
            error("error while recursively getting info for the working copy at'" + wcDir.getAbsolutePath() + "'",
                  svne);
        }
        System.out.println();
        ...
        System.out.println("Recursively scheduling a new directory '" + aNewDir.getAbsolutePath()
                           + "' for addition...");
        try {
            /**
        ...
        try {
            /**
             * gets and shows status information for the WC directory. status will be recursive on wcDir, will also
             * cover the repository, won't cover unmodified entries, will disregard 'svn:ignore' property ignores (if
        ...
        try {
            /**
             * commits changes in wcDir to the repository with not leaving items locked (if any) after the commit
             * succeeds; this will add aNewDir & aNewFile to the repository.
             */
            committedRevision = commit(wcDir, false,
                                       "'/newDir' with '/newDir/newFile.txt' were added").getNewRevision();
        } catch (SVNException svne) {
        ...
        try {
            ...
            committedRevision = copy(url, copyURL, false,
                                     "remotely copying '" + url + "' to '" + copyURL + "'").getNewRevision();
        } catch (SVNException svne) {
        ...
        try {
            /**
             * lastly commits changes in wcDir to the repository; all items that were locked by the user (if any) will
             * be unlocked after the commit succeeds; this commit will remove aNewDir from the repository.
             */
            committedRevision = commit(wcDir, false, "deleting '" + aNewDir.getAbsolutePath()
     ...
     */
    private static long switchToURL(File wcPath, SVNURL url, SVNRevision updateToRevision,
                                    boolean isRecursive) throws SVNException {
        SVNUpdateClient updateClient = ourClientManager.getUpdateClient();
        /**
    ...
    /**
     * Duplicates srcURL to dstURL (URL->URL)in a repository remembering history. Like 'svn copy srcURL dstURL -m "some
     * comment"' command. It's done by invoking doCopy(SVNURL srcURL, SVNRevision srcRevision, SVNURL dstURL, boolean
     * isMove, String commitMessage) which takes the following parameters: srcURL - a source URL that is to be copied;
     * srcRevision - a definite revision of srcURL dstURL - a URL where srcURL will be copied; if srcURL & dstURL are
     * both directories then there are two cases: a) dstURL already exists - then doCopy(..) will duplicate the entire
     * source directory and put it inside dstURL (for example, consider srcURL = svn://localhost/rep/MyRepos, dstURL =
     * svn://localhost/rep/MyReposCopy, in this case if doCopy(..) succeeds MyRepos will be in MyReposCopy -
     * svn://localhost/rep/MyReposCopy/MyRepos); b) dstURL doesn't exist yet - then doCopy(..) will create a directory
     * and recursively copy entries from srcURL into dstURL (for example, consider the same srcURL =
     * svn://localhost/rep/MyRepos, dstURL = svn://localhost/rep/MyReposCopy, in this case if doCopy(..) succeeds
     * MyRepos entries will be in MyReposCopy, like: svn://localhost/rep/MyRepos/Dir1 ->
     * svn://localhost/rep/MyReposCopy/Dir1...); isMove - if false then srcURL is only copied to dstURL what corresponds
     * to 'svn copy srcURL dstURL -m "some comment"'; but if it's true then srcURL will be copied and deleted - 'svn
     * move srcURL dstURL -m "some comment"'; commitMessage - a commit log message since URL->URL copying is immediately
     * committed to a repository.
     */
    private static SVNCommitInfo copy(SVNURL srcURL, SVNURL dstURL, boolean isMove,
                                      String commitMessage) throws SVNException {
        /**
         * SVNRevision.HEAD means the latest revision. Returns SVNCommitInfo containing information on the new revision
        ...
        return ourClientManager.getCopyClient().doCopy(new SVNCopySource[] {}, dstURL, isMove, false, false,
                                                       commitMessage, null);
    }

/*
 * ==================================================================== Copyright (c) 2004-2011 TMate Software Ltd. All
...
public class Replicate {
    ...
    public static void main(String[] args) {
        /**
         * Default values: source and target repository paths
         */
        ...
        try {
            if (srcUrl != null) {
                ...
                if ("file".equals(srcURL.getProtocol())) {
                    ...
                    if (!srcReposDir.exists()) {
                        /**
                         * it's a local access scheme and src path does not exist - we'll need to create something
                         */
                        createSrcRepos = true;
                        ...
                        if (srcRepository.getLatestRevision() == 0) {
                            /**
                             * it's a local access scheme, src path already exists, but seems to be an empty repository
            ...
            /**
             * For the target repository we need to enable revision property changes.
             */
            ...
            /**
             * getFullMessage() will give us a full tree of SVNException errors.
             */
        ...
        /**
         * Deault auth manager is used to cache a username in the default Subversion credentials storage area.
         */
        ...
        try {
            ...
            System.out.println();
            /**
             * Using the DisplayRepositoryTree example to show the source repository tree in the latest revision.
             */
            DisplayRepositoryTree.listEntries(srcRepository, "");
            System.out.println();
        } catch (SVNException svne) {
            System.err.println("An error occurred while accessing source repository: "
                               + svne.getErrorMessage().getFullMessage());
            System.exit(1);
        }
        ...
        try {
            ...
            try {
                ...
                /**
                 * ...else the server does not support replay functionality, we try our own replication engine. It could
                 * have happened if we used http:// or svn:// access protocols, and the server had binaries younger than
                 * the Subversion 1.4.0
                 */

                /**
                 * Create a new SVNRepository for the target url, since the one we used may be locked by a commit editor
                 * it provides for committing, in this case we'll get an SVNException saying that '..methods of
                 * SVNRepository are not reenterable'.
                 */
                tgtRepository = SVNRepositoryFactory.create(tgtURL);
    ...
    /**
     * This is a very simplified form of SVNAdminClient.doInitialize() that we use here only for replay functionality
     * demonstration.
     */
    private static void initializeRepository(SVNRepository fromRepos, SVNRepository toRepos) throws SVNException {
        /**
         * Initialization means we need to set necessary svn:sync- properties on revision 0 of the destination
         * repository. But since our program is just an example, we copy only revision properties from the source
    ...
    /**
     * This method does not make all necessary checks that would allow us to copy revision properties correctly under
     * any conditions... But it's suitable enough for our example.
     */
    private static void copyRevisionProperties(SVNRepository fromRepository, SVNRepository toRepository,
    ...
    /**
     * This is a very simplified form of SVNAdminClient.doSynchronize() that we use here only for replay functionality
     * demonstration.
     */
    private static long synchronizeRepository(SVNRepository fromRepos, SVNRepository toRepos) throws SVNException {
        ...
        for (long currentRev = lastMergedRevision + 1; currentRev <= fromLatestRevision; currentRev++) {
            ...
            if (info.getNewRevision() != currentRev) {
                System.err.println("Commit created rev " + info.getNewRevision() + " but should have created "
                                   + currentRev);
                System.exit(1);
            }
    ...
    private static void populateSourceRepository(SVNRepository srcRepository) throws SVNException {
        /**
         * Simple repository tree to create. Each entry will be added in its own revision.
         */
        String dirA = "dirA";
    ...
    /**
     * This routine makes synchronization between two repositories using SVNKit's own feature - replicator.
     */
    private static long replicateRepository(SVNRepository srcRepository,
                                            SVNRepository tgtRepository) throws SVNException {
        ...
        /**
         * Replicates the source repository into the target one starting with the first revision and up to the latest
         * revision of the source repository. The same result is reached if you call:
         * replicator.replicateRepository(srcRepository, tgtRepository, -1, -1) The return value is the total number of
